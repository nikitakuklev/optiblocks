...
from PyQt5.QtWidgets import QAbstractSpinBox, QSizePolicy, QWidget
from pyqtgraph import SignalProxy, functions as fn
...
class NumberBox(QAbstractSpinBox):
    """
    ...
    **Signals:**
    ...
    def __init__(self, parent=None, value=0.0, **kwargs):
        ...
        **Arguments:**
        ...
        self.opts = {
            ...
            'int': False,
            ## Set True to force value to be integer. If True, 'step' is rounded to the nearest integer or defaults to 1.
            'finite': True,

            'prefix': '',  ## string to be prepended to spin box value
            'suffix': '',
            'siPrefix': False,
            ## Set to True to display numbers with SI prefix (ie, 100pA instead of 1e-10A)
            ...
            'format': "{prefix}{prefixGap}{scaledValue}{suffixGap}{siPrefix}{suffix}",
        ...
        self.val = decimal.Decimal(
                str(value))  ## Value is precise decimal. Ordinary math not allowed.
        self.updateText()
    ...
    #     elif a1.type() == QEvent.MouseButtonDblClick:
    ...
    def focusInEvent(self, ev):
        super(NumberBox, self).focusInEvent(ev)
        self.selectNumber()
    ...
    def widgetGroupInterface(self):
        return (self.valueChanged, NumberBox.value, NumberBox.setValue)
    ...
    def stepBy(self, n):
        ...
        for i in range(int(abs(n))):
            if self.opts['dec']:
                if val == 0:
                    ...
                    fudge = decimal.Decimal('1.01') ** (
                            s * vs)  ## fudge factor. at some places, the step size depends on the step sign.
        ...
        self.setValue(val,
                      delaySignal=True)  ## note all steps (arrow buttons, wheel, up/down keys..) emit delayed signals only.
    ...
    def formatText(self, prev=None):
        ...
        if self.opts['siPrefix'] is True:
            ...
            if self.val == 0 and prev is not None:
                ...
                (s, p) = fn.siScale(val)
            parts = {'value': val, 'suffix': suffix, 'decimals': decimals, 'siPrefix': p,
                     'scaledValue': s * val, 'prefix': prefix
                     }

        else:
            # no SI prefix /suffix requested; scale is 1
            parts = {'value': val, 'suffix': suffix, 'decimals': decimals, 'siPrefix': '',
                     'scaledValue': val, 'prefix': prefix
                     }
    ...
    def editingFinishedEvent(self):
        ...
        if val == self.val:
            return
        self.setValue(val,
                      delaySignal=False)  ## allow text update so that values are reformatted pretty-like
